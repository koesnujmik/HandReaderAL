...
class CTCTrainer(AbstractTrainer):
    """
    ...
    def __init__(
        ...
        scheduler=None,
        grad_accumulate: float = 1.0,
        path_to_states: str = None,
    ) -> None:
        """
        Initializes the CTCTrainer.
        ...
        ----------
        best_lev : int
            The best Levenshtein accuracy obtained thus far.
        ...
        writer : tensorboard.SummaryWriter
            A writer for logging metrics to TensorBoard.
    ...
    def train_step(self, batch):
        ...
        loss, and updates model weights using the optimizer.
        ...
        metrics : dict
            ...
            (if `get_metric_train` is True), phrase predictions, and ground truth phrases.
        """
        ...
        if self.iter % self.grad_accumulate == 0:
            self.optimizer.step()
            self.optimizer.zero_grad()
        ...
        if self.get_metric_train:
            ...
            for i in range(len(outputs)):
                curr_pred = self.decoder.greedy_decode(probs[i].detach().cpu().numpy())
    ...
    def validate_step(self, batch):
        ...
        This function takes a batch of inputs and targets, forwards them through the model,
        calculates the loss using the specified loss function, and evaluates the model's
        performance by predicting phrases and comparing them to ground truth phrases.

        Parameters
        ----------
        batch : tuple
            A tuple containing the inputs, targets, input lengths, and target lengths.

        Returns
        -------
        dict
            A dictionary containing the validation loss, Levenshtein accuracy, predicted
            phrases, and ground truth phrases.
        """
        inputs, targets, input_lengths, target_lengths = batch
        inputs, targets, input_lengths, target_lengths = (
            inputs.to(self.device),
    ...
    def fit(self, train_loader, val_loader=None, epochs=1):
        ...
        This method will train the model using the training loader and optionally
        validate it using the validation loader. The method will also save the best
        model state dict based on the Levenshtein accuracy on the validation set.
        ...
        val_loader : DataLoader, optional
            The DataLoader for the validation set. Default is None.
        epochs : int, optional
    ...
    def on_epoch_end(
        ...
        """
        Logs metrics and saves checkpoints at the end of each epoch.

        This method logs training and validation metrics, including losses and
        Levenshtein accuracy. It also logs predicted and ground truth phrases
        for both training and validation datasets. If a learning rate scheduler
        is used, it steps the scheduler and logs the learning rate. Finally, it
        ...
        preds_val : list[str]
            List of predicted phrases for the validation dataset.
        gts_val : list[str]
            List of ground truth phrases for the validation dataset.
        train_metric : float, optional
            The training metric (e.g., Levenshtein accuracy) for the current epoch. Default is None.
        preds_train : list[str], optional
            List of predicted phrases for the training dataset. Default is None.
        gts_train : list[str], optional
            List of ground truth phrases for the training dataset. Default is None.
        """
    ...
    def save_checkpoint(self, name: str, path_to_save: str, model, epoch: int = None):
        ...
        This method saves a checkpoint of the model state, including the model
        weights, optimizer state, learning rate scheduler state, best Levenshtein
        accuracy, random state, and CUDA random state. If the epoch is not None,
        it is saved as well.
    ...
    def log_metrics(self, metric_name: str, metric_value: float, epoch: int):
        """
        Logs a metric value to the tensorboard writer.
