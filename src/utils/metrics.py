...
def get_score(phrase_gt, phrase_preds):
    """
    Get score for phrase prediction
    Parameters
    ----------
    phrase_gt: list of ground truth phrases
    phrase_preds: list of predicted phrases
    Returns
    -------
    score: float
    ...
    # add 100 to percentage
    score = 100 * (N.sum() - D.sum()) / N.sum()

    return score


def iterative_levenshtein(s, t, costs=(1, 1, 1)):
    """Computes Levenshtein distance between the strings s and t. For all i and j, dist[i,j] will
    contain the Levenshtein distance between the first i characters of s and the first j characters
    of t.
...
def compute_acc(preds, labels, costs=(7, 7, 10)):
    ...
    """
    Compute accuracy for a list of predictions and labels
    Parameters
    ----------
    preds: list of predicted phrases
    labels: list of ground truth phrases
    costs: a tuple or a list with three integers (d, i, s)
           where d defines the costs for a deletion
                 i defines the costs for an insertion and
                 s defines the costs for a substitution
    ...
    for i, _ in enumerate(preds):
        _, D, S, I = iterative_levenshtein(preds[i], labels[i], costs)
        Ns += len(labels[i])
        Ds += D
        Ss += S
        Is += I
