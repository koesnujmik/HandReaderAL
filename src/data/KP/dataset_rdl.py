...
class TokenizerRU:
    ...
    def __init__(self, ctc: bool, autoreg: bool):
        """
        Initializes the TokenizerRU with the specified settings.

        Parameters
        ----------
        ctc : bool
            If True, includes a blank symbol for CTC training. The blank symbol is added at index 0.
        autoreg : bool
            If True, operates in autoregressive mode where each character is mapped to an index.
            If both `ctc` and `autoreg` are False, this defaults to a character-to-index mapping.
        ...
        self.index_map = {}
        if self.ctc:
            for i, ch in enumerate([BLANK_SYMBOL] + list(chars)):
                self.char_map[ch] = i
                self.index_map[i] = ch
...
class Dataset_RDL(Dataset):

    """
    Dataset class for the RDL dataset.

    This class handles loading, processing, and augmenting data for the RDL dataset, including applying flip augmentations, text tokenization, and data preprocessing.

    Attributes
    ----------
    df : pd.DataFrame
        The dataframe containing dataset information (e.g., file paths and processed text).
    mode : str
        The mode of the dataset, either 'train', 'val', or 'test'. Determines which augmentations to apply.
    aug : callable, optional
        A function or transform to apply additional augmentations. Defaults to None.
    dec : Tokenizer
        The tokenizer used for converting text (phrases) to token ids.
    data_folder : str
        Path to the folder containing the raw data files (e.g., .npy files).
    flip_aug : float
        The probability of applying a flip augmentation.
    filtered_columns : list of int, optional
        List of column indices after filtering based on the provided substrings in `inds_to_filter`.
    xyz_landmarks : np.ndarray
        A numpy array of XYZ coordinates for the landmarks.
    flip_array : np.ndarray
        Array of indices for flipped landmarks based on symmetry data.
    processor : Preprocessing
        The preprocessing instance responsible for applying preprocessing operations on the data.
    """
    ...
    def __init__(
        ...
        """
        Initializes the RDL dataset with data for landmarks, augmentation, and tokenization.
        ...
        ----------
        df : pd.DataFrame
            The dataframe containing dataset information.
        mode : str
            The mode of the dataset, either 'train', 'val', or 'test'.
        aug : callable, optional
            A function or transform to apply additional augmentations.
        dec : Tokenizer
            The tokenizer used for converting text (phrases) to token ids.
        data_folder : str
            Path to the folder containing the raw data files (e.g., .npy files).
    ...
    def __getitem__(self, idx):
        ...
        tuple
            A tuple containing:
            - data : torch.Tensor
                The processed data sample as a float tensor.
            - token_ids : list[int]
    ...
    def load_one(self, path_to_file: str):
        ...
        """
        path = self.data_folder + f"/{path_to_file}"
        data = np.load(path)  # seq_len, 3* nlandmarks
        return data


class Dataset_RDL_KP_Datamodule:
    """
    Data module for the RDL dataset with Keypoint data.

    This class handles the setup, transformation, and data loading for the RDL dataset, including handling different dataset splits (train, validation, test) and applying data augmentation transforms to the training data.
    ...
    data_folder : str
        The path to the folder containing the raw data files (e.g., .npy files).
    tokenizer : Tokenizer
        The tokenizer used for converting phrases to token ids.
    inference_args : str
        Path to the inference arguments JSON file, which contains the selected columns.
    flip_aug : float
        The probability of applying a flip augmentation to the data.
    symmetry_fp : str
        Path to the symmetry CSV file for landmark flipping.
    df : str
        Path to the CSV file containing the dataset split information.
    ...
    transfomrs_train : albumentations.Compose
        The set of augmentation transformations to apply to the training data.
    ...
    def __init__(
        ...
        """
        Initializes the data module with configurations for data processing and augmentation.
        ...
        inference_args : str
            Path to the JSON file containing selected columns for inference.
        batch_size : int
            The number of samples in each batch during data loading.
        num_workers : int
            The number of subprocesses to use for data loading.
        symmetry_fp : str
            Path to the CSV file containing symmetry information for landmark flipping.
        flip_aug : float
            Probability of applying flip augmentation to the data.
        df : str
            Path to the CSV file containing dataset split information.
        tokenizer : Tokenizer
            The tokenizer object for mapping labels to integers and vice versa.
        inds_to_filter : list[str]
            List of substrings to filter columns for landmark data.
        """
        ...
        self.transfomrs_train = A.Compose(
            [
                ...
                A.TemporalMask(size=(0.2, 0.4), mask_value=0.0, p=0.5),
                A.SpatialMask(size=(0.05, 0.1), mask_value=0.0, mode="relative", p=0.5),
            ]
        )
    ...
    def get_dataset(self, stage: str):
        ...
        if stage == "train":
            ...
            LOGGER.info(f"Ininting {stage} dataset, len is {len(self.data_train)}")
            return self.data_train

        elif stage == "val":
            df = pd.read_csv(self.df)
            val_df = df[df["split"] == "val"].copy()
            self.data_val = Dataset_RDL(
                val_df,
                data_folder=self.data_folder,
                inference_args=self.inference_args,
                aug=None,
            ...
            LOGGER.info(f"Ininting {stage} dataset, len is {len(self.data_val)}")
            return self.data_val

        elif stage == "test":
            df = pd.read_csv(self.df)
            test_df = df[df["split"] == "test"].copy()
            self.data_test = Dataset_RDL(
                test_df,
                data_folder=self.data_folder,
                inference_args=self.inference_args,
                aug=None,
            ...
            LOGGER.info(f"Ininting {stage} dataset, len is {len(self.data_test)}")
